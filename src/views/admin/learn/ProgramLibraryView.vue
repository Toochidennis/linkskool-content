        ...
        <div class="program-content">
          <div class="program-header-row">
            ...
            <div class="menu-container">
              <button @click="toggleMenu(program.id)" class="menu-button">
              ...
              <Transition name="dropdown">
                <div v-if="activeMenu === program.id" class="menu-dropdown">
                  <button @click="editProgram(program)" class="menu-item">
                  ...
                  <button @click="deleteProgram(program.id)" class="menu-item menu-item-danger"
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
        <div class="modal-container">
          ...
          <div class="modal-body">
            ...
            <div class="section-header">
              <h4 class="section-title">💰 Pricing & Access</h4>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="toggle-container">
                <button type="button" @click="formData.isFree = !formData.isFree" class="toggle-button"
                  :class="{ 'toggle-active': formData.isFree }">
                  <span class="toggle-slider"></span>
                </button>
                <span class="toggle-label">{{ formData.isFree ? 'Free Program' : 'Paid Program' }}</span>
              </div>
            </div>

            <!-- Trial Settings (Only if Paid) -->
            <div v-if="!formData.isFree" class="trial-section">
              <label class="form-label">Free Trial Setup</label>

              <div class="trial-type-selector">
                <label class="radio-option">
                  <input type="radio" v-model="formData.trialType" value="watches" name="trial-type" />
                ...
                <label class="radio-option">
                  <input type="radio" v-model="formData.trialType" value="days" name="trial-type" />
                  <span class="radio-label">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div v-if="formData.trialType" class="form-group">
                <label class="form-label">
                  {{ formData.trialType === 'watches' ? 'Number of Free Views' : 'Number of Trial Days' }}
                </label>
                <input v-model.number="formData.trialValue" type="number" min="1" class="form-input"
                  :placeholder="formData.trialType === 'watches' ? 'e.g., 5 videos' : 'e.g., 7 days'" />
              </div>
            </div>
...
const isFormValid = computed(() => {
  ...
  // If paid and trial is set, must have trial value
  if (!formData.value.isFree && formData.value.trialType) {
    return basicValid && formData.value.trialValue !== undefined && formData.value.trialValue > 0;
  }
...
const confirmDelete = async () => {
  if (programToDelete.value === null) return;
  if (deleteLoadingId.value === programToDelete.value) return;

  const programId = programToDelete.value;
