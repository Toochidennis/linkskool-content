  ...
  <div class="lesson-form-page">
    ...
    <div class="form-header">
      ...
      <div class="header-actions">
        <button class="save-btn save-draft" @click="handleSave('draft')" :disabled="isSaving">
          <svg v-if="isSaving && pendingStatus === 'draft'" class="spinner-icon" viewBox="0 0 24 24" fill="none"
            stroke="currentColor">
        ...
        <button class="save-btn save-publish" @click="handleSave('published')" :disabled="isSaving">
          <svg v-if="isSaving && pendingStatus === 'published'" class="spinner-icon" viewBox="0 0 24 24" fill="none"
            stroke="currentColor">
            <circle cx="12" cy="12" r="10" stroke-width="2" />
          </svg>
          <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path d="M5 12l4 4 10-10" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
    ...
    <div class="form-content">
      <div class="lesson-form">
        ...
        <div class="form-section">
          ...
          <div class="form-row">
            <div class="form-group" :class="{ 'has-error': fieldErrors.title }">
              ...
              <input v-model="localLesson.title" type="text" class="form-input" placeholder="Enter lesson title..."
                @blur="handleBlur('title')" @input="handleInput('title')" />
          ...
          <div class="form-row">
            <div class="form-group full" :class="{ 'has-error': fieldErrors.goals }">
              ...
              <div class="rich-editor-wrapper">
                <RichTextEditor :model-value="localLesson.goals" placeholder="What is the main goal of this lesson?"
                  @update:model-value="
                    (value) => {
            ...
            <div class="form-group full">
              ...
              <div class="rich-editor-wrapper">
                <RichTextEditor :model-value="localLesson.objectives"
                  placeholder="Enter learning objectives... (bullet points, bold, italic, etc.)" @update:model-value="
                    (value) => {
            ...
            <div class="form-group full">
              ...
              <div class="rich-editor-wrapper">
                <RichTextEditor :model-value="localLesson.writeupContent"
                  placeholder="Write detailed material content, notes, or reading materials..." @update:model-value="
                    (value) => {
        ...
        <div v-if="!localLesson.isFinalLesson" class="form-section">
          <h4 class="section-title">Live Session</h4>
          <div class="form-row">
            <div class="form-group full">
              <label class="form-label">URL</label>
              <input v-model="zoomInfo.url" type="url" class="form-input" placeholder="https://zoom.us/j/..."
                @blur="handleZoomUrlBlur" @input="handleZoomUrlInput" />
              <p class="form-hint">Optional. If provided, start and end time are required.</p>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input v-model="zoomInfo.meetingId" type="text" class="form-input" placeholder="Enter meeting ID..." />
            </div>
            <div class="form-group">
              <label class="form-label">Passcode</label>
              <input v-model="zoomInfo.passcode" type="text" class="form-input" placeholder="Enter passcode..." />
            </div>
          </div>

          <div class="form-row">
            <div class="form-group" :class="{ 'has-error': fieldErrors.zoomStartTime }">
              <label class="form-label">Start Time</label>
              <input v-model="zoomInfo.startTime" type="datetime-local" class="form-input"
                @blur="handleBlur('zoomStartTime')" @input="handleInput('zoomStartTime')" />
              <span v-if="fieldErrors.zoomStartTime" class="error-message">{{
                fieldErrors.zoomStartTime
              }}</span>
            </div>
            <div class="form-group" :class="{ 'has-error': fieldErrors.zoomEndTime }">
              <label class="form-label">End Time</label>
              <input v-model="zoomInfo.endTime" type="datetime-local" class="form-input"
                @blur="handleBlur('zoomEndTime')" @input="handleInput('zoomEndTime')" />
              <span v-if="fieldErrors.zoomEndTime" class="error-message">{{
                fieldErrors.zoomEndTime
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="form-row">
            <div class="form-group full">
              ...
              <div class="status-toggle">
                <button type="button" class="status-option draft" :class="{ active: localLesson.status === 'draft' }"
                  @click="setStatus('draft')">
                  Draft
                </button>
                <button type="button" class="status-option published"
                  :class="{ active: localLesson.status === 'published' }" @click="setStatus('published')">
                  Published
                </button>
          ...
          <div class="form-row">
            <div v-if="!localLesson.isFinalLesson" class="form-group" :class="{ 'has-error': fieldErrors.lessonDate }">
              <label class="form-label">Lesson Date<span class="required">*</span></label>
              <input v-model="localLesson.lessonDate" type="date" class="form-input" @blur="handleBlur('lessonDate')"
                @change="handleInput('lessonDate')" />
...
// Computed property to safely access zoomInfo (writable)
const zoomInfo = computed({
  get: () => localLesson.value.zoomInfo ?? createEmptyZoomInfo(),
  set: (value) => {
    localLesson.value.zoomInfo = value
  },
})
...
onMounted(() => {
  ...
  if (mode === 'edit' && lessonData) {
    ...
    const zoomInfoRaw = parsedLesson.zoomInfo || {}
    localLesson.value = {
      ...parsedLesson,
...
const validateField = (fieldName: string): string => {
  switch (fieldName) {
    ...
    case 'certificateFile':
      ...
      return ''
    case 'zoomStartTime': {
      const url = localLesson.value.zoomInfo?.url?.trim()
      if (url && !localLesson.value.zoomInfo?.startTime?.trim()) {
        return 'Start time is required when URL is provided'
      }
      return ''
    }
    case 'zoomEndTime': {
      const url = localLesson.value.zoomInfo?.url?.trim()
      if (url && !localLesson.value.zoomInfo?.endTime?.trim()) {
        return 'End time is required when URL is provided'
      }
      return ''
    }
...
const handleZoomUrlBlur = () => {
  if (!localLesson.value.zoomInfo?.url?.trim()) {
    clearFieldError('zoomStartTime')
    clearFieldError('zoomEndTime')
    return
  }
  handleBlur('zoomStartTime')
  handleBlur('zoomEndTime')
}

const handleZoomUrlInput = () => {
  if (!localLesson.value.zoomInfo?.url?.trim()) {
    clearFieldError('zoomStartTime')
    clearFieldError('zoomEndTime')
  }
